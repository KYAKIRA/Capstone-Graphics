...
namespace PizzaBox{
	...
	public:
		...
		static inline float Sqrt(float value_){
			//Up-converts argument to double because double sqrt is 5x faster than the float version
		...
		static inline float Sin(float angle_){
			return sin(ConvertToRadians(angle_));
		}

		static inline float Cos(float angle_){
			return cos(ConvertToRadians(angle_));
		}

		static inline float Tan(float angle_){
			return tan(ConvertToRadians(angle_));
		}

		static inline float Asin(float sin_){
			return ConvertToDegrees(asin(sin_));
		}

		static inline float Acos(float cos_){
			return ConvertToDegrees(acos(Clamp(-1.0f, 1.0f, cos_)));
		}

		static inline float Atan(float tan_){
			return ConvertToDegrees(atan(tan_));
		}

		static inline float Atan2(float a_, float b_){
			return ConvertToDegrees(atan2(a_, b_));
		}
