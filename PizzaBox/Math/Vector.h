...
namespace PizzaBox{
	...
	struct Vector3{
		...
		explicit Vector3(const aiVector3D& v_);
		~Vector3();
		...
		static Vector3 Reflect(const Vector3& v_, const Plane& p_);
		...
		static float Distance(const Vector3& v_, const Plane& p_);
		//Get the distance between a sphere and a plane
		static float Distance(const Sphere& s_, const Plane& p_);
		...
		//Functions for converting to FMOD Vectors, for convenience
		FMOD_VECTOR ToFMOD() const;
		static FMOD_VECTOR ToFMOD(const Vector3& v_);
	};
