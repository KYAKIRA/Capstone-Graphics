        ...
        if (reportContacts) {
            ...
            for (uint f = 0; f < polyhedron->getNbFaces(); f++) {
                ...
                if (isFaceNormalInContactDirection && areOrthogonalVectors(faceNormalWorld, capsuleInnerSegmentDirection)
                    ...
                    const Transform capsuleToWorldLocal = isCapsuleShape1 ? narrowPhaseInfo->shape1ToWorldTransform : narrowPhaseInfo->shape2ToWorldTransform;
                    const Transform polyhedronToCapsuleTransform = capsuleToWorldLocal.getInverse() * polyhedronToWorld;

                    // Compute the end-points of the inner segment of the capsule
                    const Vector3 capsuleSegALocal(0, -capsuleShape->getHeight() * decimal(0.5), 0);
                    const Vector3 capsuleSegBLocal(0, capsuleShape->getHeight() * decimal(0.5), 0);
                    ...
                    const Vector3 capsuleSegAPolyhedronSpace = capsuleToPolyhedronTransform * capsuleSegALocal;
                    const Vector3 capsuleSegBPolyhedronSpace = capsuleToPolyhedronTransform * capsuleSegBLocal;
