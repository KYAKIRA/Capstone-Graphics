...
HingeJoint::HingeJoint(uint id, const HingeJointInfo& jointInfo)
           : Joint(id, jointInfo), mImpulseTranslation(0, 0, 0), mImpulseRotation(0, 0),
             ...
             mMotorSpeed(jointInfo.motorSpeed), mMaxMotorTorque(jointInfo.maxMotorTorque), mInverseMassMatrixMotor(), mInverseMassMatrixLimitMotor(), mBUpperLimit(), mBLowerLimit() {
...
void HingeJoint::solveVelocityConstraint(const ConstraintSolverData& constraintSolverData) {
    ...
    if (mIsLimitEnabled) {
        ...
        if (mIsLowerLimitViolated) {
            ...
            const Vector3 angularImpulseBody1Local = -deltaLambdaLower * mA1;

            // Apply the impulse to the body 1
            w1 += mI1 * angularImpulseBody1Local;

            // Compute the impulse P=J^T * lambda for the lower limit constraint of body 2
            const Vector3 angularImpulseBody2Local = deltaLambdaLower * mA1;
        ...
        if (mIsUpperLimitViolated) {
            ...
            const Vector3 angularImpulseBody1Local = deltaLambdaUpper * mA1;

            // Apply the impulse to the body 1
            w1 += mI1 * angularImpulseBody1Local;

            // Compute the impulse P=J^T * lambda for the upper limit constraint of body 2
            const Vector3 angularImpulseBody2Local = -deltaLambdaUpper * mA1;
    ...
    if (mIsMotorEnabled) {
        ...
        const Vector3 angularImpulseBody1Local = -deltaLambdaMotor * mA1;

        // Apply the impulse to the body 1
        w1 += mI1 * angularImpulseBody1Local;

        // Compute the impulse P=J^T * lambda for the motor of body 2
        const Vector3 angularImpulseBody2Local = deltaLambdaMotor * mA1;
...
void HingeJoint::solvePositionConstraint(const ConstraintSolverData& constraintSolverData) {
    ...
    if (mIsLimitEnabled) {
        ...
        if (mIsLowerLimitViolated) {
            ...
            // Compute the impulse P=J^T * lambda of body 1
            const Vector3 angularImpulseBody1Local = -lambdaLowerLimit * mA1;
            ...
            // Compute the impulse P=J^T * lambda of body 2
            const Vector3 angularImpulseBody2Local = lambdaLowerLimit * mA1;
        ...
        if (mIsUpperLimitViolated) {
            ...
            // Compute the impulse P=J^T * lambda of body 1
            const Vector3 angularImpulseBody1Local = lambdaUpperLimit * mA1;
            ...
            // Compute the impulse P=J^T * lambda of body 2
            const Vector3 angularImpulseBody2Local = -lambdaUpperLimit * mA1;
