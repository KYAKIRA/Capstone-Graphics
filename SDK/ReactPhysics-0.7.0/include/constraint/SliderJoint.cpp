...
SliderJoint::SliderJoint(uint id, const SliderJointInfo& jointInfo)
            : Joint(id, jointInfo), mImpulseTranslation(0, 0), mImpulseRotation(0, 0, 0),
              ...
              mMaxMotorForce(jointInfo.maxMotorForce), mInverseMassMatrixMotor(), mInverseMassMatrixLimit(), mBUpperLimit(), mBLowerLimit(){
...
void SliderJoint::solveVelocityConstraint(const ConstraintSolverData& constraintSolverData) {
    ...
    if (mIsLimitEnabled) {
        ...
        if (mIsLowerLimitViolated) {
            ...
            const Vector3 linearImpulseBody1Local = -deltaLambdaLower * mSliderAxisWorld;
            const Vector3 angularImpulseBody1Local = -deltaLambdaLower * mR1PlusUCrossSliderAxis;

            // Apply the impulse to the body 1
            v1 += inverseMassBody1 * linearImpulseBody1Local;
            w1 += mI1 * angularImpulseBody1Local;

            // Compute the impulse P=J^T * lambda for the lower limit constraint of body 2
            const Vector3 linearImpulseBody2Local = deltaLambdaLower * mSliderAxisWorld;
            const Vector3 angularImpulseBody2Local = deltaLambdaLower * mR2CrossSliderAxis;

            // Apply the impulse to the body 2
            v2 += inverseMassBody2 * linearImpulseBody2Local;
            w2 += mI2 * angularImpulseBody2Local;
        }
        ...
        if (mIsUpperLimitViolated) {
            ...
            const Vector3 linearImpulseBody1Local = deltaLambdaUpper * mSliderAxisWorld;
            const Vector3 angularImpulseBody1Local = deltaLambdaUpper * mR1PlusUCrossSliderAxis;
            ...
            const Vector3 linearImpulseBody2Local = -deltaLambdaUpper * mSliderAxisWorld;
            const Vector3 angularImpulseBody2Local = -deltaLambdaUpper * mR2CrossSliderAxis;
    ...
    if (mIsMotorEnabled) {
        ...
        const Vector3 linearImpulseBody1Local = deltaLambdaMotor * mSliderAxisWorld;

        // Apply the impulse to the body 1
        v1 += inverseMassBody1 * linearImpulseBody1Local;

        // Compute the impulse P=J^T * lambda for the motor of body 2
        const Vector3 linearImpulseBody2Local = -deltaLambdaMotor * mSliderAxisWorld;
...
void SliderJoint::solvePositionConstraint(const ConstraintSolverData& constraintSolverData) {
    ...
    if (mIsLimitEnabled) {
        ...
        if (mIsLowerLimitViolated) {
            ...
            // Compute the impulse P=J^T * lambda for the lower limit constraint of body 1
            const Vector3 linearImpulseBody1Local = -lambdaLowerLimit * mSliderAxisWorld;
            const Vector3 angularImpulseBody1Local = -lambdaLowerLimit * mR1PlusUCrossSliderAxis;

            // Apply the impulse to the body 1
            const Vector3 v1Local = inverseMassBody1 * linearImpulseBody1Local;
            const Vector3 w1Local = mI1 * angularImpulseBody1Local;

            // Update the body position/orientation of body 1
            x1 += v1Local;
            q1 += Quaternion(0, w1Local) * q1 * decimal(0.5);
            q1.normalize();

            // Compute the impulse P=J^T * lambda for the lower limit constraint of body 2
            const Vector3 linearImpulseBody2Local = lambdaLowerLimit * mSliderAxisWorld;
            const Vector3 angularImpulseBody2Local = lambdaLowerLimit * mR2CrossSliderAxis;

            // Apply the impulse to the body 2
            const Vector3 v2Local = inverseMassBody2 * linearImpulseBody2Local;
            const Vector3 w2Local = mI2 * angularImpulseBody2Local;

            // Update the body position/orientation of body 2
            x2 += v2Local;
            q2 += Quaternion(0, w2Local) * q2 * decimal(0.5);
            q2.normalize();
        }
        ...
        if (mIsUpperLimitViolated) {
            ...
            // Compute the impulse P=J^T * lambda for the upper limit constraint of body 1
            const Vector3 linearImpulseBody1Local = lambdaUpperLimit * mSliderAxisWorld;
            const Vector3 angularImpulseBody1Local = lambdaUpperLimit * mR1PlusUCrossSliderAxis;
            ...
            // Compute the impulse P=J^T * lambda for the upper limit constraint of body 2
            const Vector3 linearImpulseBody2Local = -lambdaUpperLimit * mSliderAxisWorld;
            const Vector3 angularImpulseBody2Local = -lambdaUpperLimit * mR2CrossSliderAxis;
