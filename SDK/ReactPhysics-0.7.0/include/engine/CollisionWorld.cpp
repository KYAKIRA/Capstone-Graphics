...
CollisionWorld::~CollisionWorld() {
    ...
    for (int i=static_cast<int>(mBodies.size() - 1); i >= 0; i--) {
...
bodyindex CollisionWorld::computeNextAvailableBodyId() {
    ...
    if (mFreeBodiesIds.size() != 0) {
        bodyID = static_cast<bodyindex>(mFreeBodiesIds[static_cast<luint>(mFreeBodiesIds.size() - 1)]);
        mFreeBodiesIds.removeAt(static_cast<uint>(mFreeBodiesIds.size() - 1));
    }
    else {
