...
ContactSolver::ContactSolver(MemoryManager& memoryManager, const WorldSettings& worldSettings)
              :mMemoryManager(memoryManager), mSplitLinearVelocities(nullptr),
               ...
               mIsSplitImpulseActive(true), mWorldSettings(worldSettings), mTimeStep(), mNbContactPoints(), mNbContactManifolds() {
    ...
    for (uint c=0; c<mNbContactManifolds; c++) {
        ...
        for (short int i=0; i<mContactConstraints[c].nbContacts; i++) {
            ...
            if (mIsSplitImpulseActive) {
                ...
                Vector3 linearImpulseLocal(mContactPoints[contactPointIndex].normal.x * deltaLambdaSplit,
                                      mContactPoints[contactPointIndex].normal.y * deltaLambdaSplit,
                                      mContactPoints[contactPointIndex].normal.z * deltaLambdaSplit);

                // Update the velocities of the body 1 by applying the impulse P
                mSplitLinearVelocities[mContactConstraints[c].indexBody1].x -= mContactConstraints[c].massInverseBody1 * linearImpulseLocal.x;
                mSplitLinearVelocities[mContactConstraints[c].indexBody1].y -= mContactConstraints[c].massInverseBody1 * linearImpulseLocal.y;
                mSplitLinearVelocities[mContactConstraints[c].indexBody1].z -= mContactConstraints[c].massInverseBody1 * linearImpulseLocal.z;
                ...
                // Update the velocities of the body 1 by applying the impulse P
                mSplitLinearVelocities[mContactConstraints[c].indexBody2].x += mContactConstraints[c].massInverseBody2 * linearImpulseLocal.x;
                mSplitLinearVelocities[mContactConstraints[c].indexBody2].y += mContactConstraints[c].massInverseBody2 * linearImpulseLocal.y;
                mSplitLinearVelocities[mContactConstraints[c].indexBody2].z += mContactConstraints[c].massInverseBody2 * linearImpulseLocal.z;
        ...
        // Compute the Lagrange multiplier lambda
        decimal deltaLambdaLocal = -Jv * mContactConstraints[c].inverseFriction1Mass;
        ...
        mContactConstraints[c].friction1Impulse = std::max(-frictionLimit,
                                                    std::min(mContactConstraints[c].friction1Impulse +
                                                             deltaLambdaLocal, frictionLimit));
        deltaLambdaLocal = mContactConstraints[c].friction1Impulse - lambdaTemp;

        // Compute the impulse P=J^T * lambda
        Vector3 angularImpulseBody1(-mContactConstraints[c].r1CrossT1.x * deltaLambdaLocal,
                                    -mContactConstraints[c].r1CrossT1.y * deltaLambdaLocal,
                                    -mContactConstraints[c].r1CrossT1.z * deltaLambdaLocal);
        Vector3 linearImpulseBody2(mContactConstraints[c].frictionVector1.x * deltaLambdaLocal,
                                   mContactConstraints[c].frictionVector1.y * deltaLambdaLocal,
                                   mContactConstraints[c].frictionVector1.z * deltaLambdaLocal);
        Vector3 angularImpulseBody2(mContactConstraints[c].r2CrossT1.x * deltaLambdaLocal,
                                    mContactConstraints[c].r2CrossT1.y * deltaLambdaLocal,
                                    mContactConstraints[c].r2CrossT1.z * deltaLambdaLocal);
        ...
        // Compute the Lagrange multiplier lambda
        deltaLambdaLocal = -Jv * mContactConstraints[c].inverseFriction2Mass;
        ...
        mContactConstraints[c].friction2Impulse = std::max(-frictionLimit,
                                                    std::min(mContactConstraints[c].friction2Impulse +
                                                             deltaLambdaLocal, frictionLimit));
        deltaLambdaLocal = mContactConstraints[c].friction2Impulse - lambdaTemp;

        // Compute the impulse P=J^T * lambda
        angularImpulseBody1.x = -mContactConstraints[c].r1CrossT2.x * deltaLambdaLocal;
        angularImpulseBody1.y = -mContactConstraints[c].r1CrossT2.y * deltaLambdaLocal;
        angularImpulseBody1.z = -mContactConstraints[c].r1CrossT2.z * deltaLambdaLocal;

        linearImpulseBody2.x = mContactConstraints[c].frictionVector2.x * deltaLambdaLocal;
        linearImpulseBody2.y = mContactConstraints[c].frictionVector2.y * deltaLambdaLocal;
        linearImpulseBody2.z = mContactConstraints[c].frictionVector2.z * deltaLambdaLocal;

        angularImpulseBody2.x = mContactConstraints[c].r2CrossT2.x * deltaLambdaLocal;
        angularImpulseBody2.y = mContactConstraints[c].r2CrossT2.y * deltaLambdaLocal;
        angularImpulseBody2.z = mContactConstraints[c].r2CrossT2.z * deltaLambdaLocal;
        ...
        deltaLambdaLocal = -Jv * (mContactConstraints[c].inverseTwistFrictionMass);
        frictionLimit = mContactConstraints[c].frictionCoefficient * sumPenetrationImpulse;
        ...
        mContactConstraints[c].frictionTwistImpulse = std::max(-frictionLimit,
                                                        std::min(mContactConstraints[c].frictionTwistImpulse
                                                                 + deltaLambdaLocal, frictionLimit));
        deltaLambdaLocal = mContactConstraints[c].frictionTwistImpulse - lambdaTemp;

        // Compute the impulse P=J^T * lambda
        angularImpulseBody2.x = mContactConstraints[c].normal.x * deltaLambdaLocal;
        angularImpulseBody2.y = mContactConstraints[c].normal.y * deltaLambdaLocal;
        angularImpulseBody2.z = mContactConstraints[c].normal.z * deltaLambdaLocal;
