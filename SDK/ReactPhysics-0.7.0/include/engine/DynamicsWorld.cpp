...
DynamicsWorld::~DynamicsWorld() {
    ...
    for (int i=static_cast<int>(mJoints.size() - 1); i >= 0; i--) {
        destroyJoint(mJoints[i]);
    }

    // Destroy all the rigid bodies that have not been removed
    for (int i=static_cast<int>(mRigidBodies.size() - 1); i >= 0; i--) {
...
void DynamicsWorld::initVelocityArrays() {
    ...
    uint nbBodies = static_cast<uint>(mRigidBodies.size());
...
uint DynamicsWorld::computeNextAvailableJointId() {
    ...
    if (mFreeJointsIDs.size() != 0) {
        jointId = static_cast<uint>(mFreeJointsIDs[static_cast<luint>(mFreeJointsIDs.size() - 1)]);
        mFreeJointsIDs.removeAt(static_cast<uint>(mFreeJointsIDs.size() - 1));
    }
    else {
...
void DynamicsWorld::computeIslands() {
    ...
    for (List<RigidBody*>::Iterator it = mRigidBodies.begin(); it != mRigidBodies.end(); ++it) {
        ...
        void* allocatedMemoryIsland = mMemoryManager.allocate(MemoryManager::AllocationType::Frame,
                                                              sizeof(Island));
        mIslands[mNbIslands] = new (allocatedMemoryIsland) Island(nbBodies, nbContactManifolds, static_cast<uint>(mJoints.size()),
                                                                  mMemoryManager);
